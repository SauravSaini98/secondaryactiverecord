...
class Mysql2ActiveSchemaTest < SecondaryActiveRecord::Mysql2TestCase
  include ConnectionHelper

  def setup
    SecondaryActiveRecord::Base.connection.singleton_class.class_eval do
  ...
  def test_add_index
    ...
    def (SecondaryActiveRecord::Base.connection).data_source_exists?(*); true; end
    def (SecondaryActiveRecord::Base.connection).index_name_exists?(*); false; end
  ...
  def test_index_in_create
    def (SecondaryActiveRecord::Base.connection).data_source_exists?(*); false; end

    %w(SPATIAL FULLTEXT UNIQUE).each do |type|
      expected = "CREATE TABLE `people` (#{type} INDEX `index_people_on_last_name`  (`last_name`))"
      actual = SecondaryActiveRecord::Base.connection.create_table(:people, id: false) do |t|
  ...
  def test_index_in_bulk_change
    ...
    %w(SPATIAL FULLTEXT UNIQUE).each do |type|
      ...
      actual = SecondaryActiveRecord::Base.connection.change_table(:people, bulk: true) do |t|
        t.index :last_name, type: type
      end
      assert_equal expected, actual
    end
  ...
  def test_add_timestamps
    with_real_execute do
      begin
        SecondaryActiveRecord::Base.connection.create_table :delete_me
        SecondaryActiveRecord::Base.connection.add_timestamps :delete_me, null: true
        assert column_present?("delete_me", "updated_at", "datetime")
        assert column_present?("delete_me", "created_at", "datetime")
      ensure
        SecondaryActiveRecord::Base.connection.drop_table :delete_me rescue nil
      end
    end
  end
  ...
  def test_remove_timestamps
    with_real_execute do
      begin
        SecondaryActiveRecord::Base.connection.create_table :delete_me do |t|
          t.timestamps null: true
        end
        SecondaryActiveRecord::Base.connection.remove_timestamps :delete_me, null: true
        assert !column_present?("delete_me", "updated_at", "datetime")
        assert !column_present?("delete_me", "created_at", "datetime")
      ensure
        SecondaryActiveRecord::Base.connection.drop_table :delete_me rescue nil
  ...
  def test_indexes_in_create
    SecondaryActiveRecord::Base.connection.stubs(:data_source_exists?).with(:temp).returns(false)
    SecondaryActiveRecord::Base.connection.stubs(:index_name_exists?).with(:index_temp_on_zip).returns(false)

    expected = "CREATE TEMPORARY TABLE `temp` ( INDEX `index_temp_on_zip`  (`zip`)) AS SELECT id, name, zip FROM a_really_complicated_query"
    actual = SecondaryActiveRecord::Base.connection.create_table(:temp, temporary: true, as: "SELECT id, name, zip FROM a_really_complicated_query") do |t|
      t.index :zip
    end
  ...
  private
    ...
    def method_missing(method_symbol, *arguments)
      SecondaryActiveRecord::Base.connection.send(method_symbol, *arguments)
    end

    def column_present?(table_name, column_name, type)
      results = SecondaryActiveRecord::Base.connection.select_all("SHOW FIELDS FROM #{table_name} LIKE '#{column_name}'")
