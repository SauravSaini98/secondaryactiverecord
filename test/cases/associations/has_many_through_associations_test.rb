...
class HasManyThroughAssociationsTest < SecondaryActiveRecord::TestCase
  ...
  def test_ordered_has_many_through
    person_prime = Class.new(SecondaryActiveRecord::Base) do
  ...
  def test_should_raise_exception_for_destroying_mismatching_records
    assert_no_difference ["Person.count", "Reader.count"] do
      assert_raise(SecondaryActiveRecord::AssociationTypeMismatch) { posts(:welcome).people.destroy(posts(:thinking)) }
    end
  end
  ...
  def test_create_on_new_record
    ...
    error = assert_raises(SecondaryActiveRecord::RecordNotSaved) { p.people.create(first_name: "mew") }
    assert_equal "You cannot call create unless the parent is saved", error.message

    error = assert_raises(SecondaryActiveRecord::RecordNotSaved) { p.people.create!(first_name: "snow") }
    assert_equal "You cannot call create unless the parent is saved", error.message
  end
  ...
  def test_associate_with_create_bang_and_invalid_options
    firm = companies(:first_firm)
    assert_no_difference("firm.developers.count") { assert_raises(SecondaryActiveRecord::RecordInvalid) { firm.developers.create!(name: "0") } }
  end
  ...
  def test_push_with_invalid_record
    firm = companies(:first_firm)
    assert_raises(SecondaryActiveRecord::RecordInvalid) { firm.developers << Developer.new(name: "0") }
  end
  ...
  def test_push_with_invalid_join_record
    repair_validations(Contract) do
      ...
      assert_raises(SecondaryActiveRecord::RecordInvalid) { firm.developers << lifo }
  ...
  def test_get_ids_for_has_many_through_with_conditions_should_not_preload
    ...
    assert_not_called(SecondaryActiveRecord::Associations::Preloader, :new) do
  ...
  def test_collection_singular_ids_setter_raises_exception_when_invalid_ids_set
    ...
    e = assert_raises(SecondaryActiveRecord::RecordNotFound) { company.developer_ids = ids }
  ...
  def test_collection_singular_ids_through_setter_raises_exception_when_invalid_ids_set
    ...
    e = assert_raises(SecondaryActiveRecord::RecordNotFound) { author.essay_category_ids = ids }
  ...
  def test_incorrectly_ordered_through_associations
    assert_raises(SecondaryActiveRecord::HasManyThroughOrderError) do
