...
class AutomaticInverseFindingTests < SecondaryActiveRecord::TestCase
...
class InverseAssociationTests < SecondaryActiveRecord::TestCase
  def test_should_allow_for_inverse_of_options_in_associations
    assert_nothing_raised do
      Class.new(SecondaryActiveRecord::Base).has_many(:wheels, inverse_of: :car)
    end

    assert_nothing_raised do
      Class.new(SecondaryActiveRecord::Base).has_one(:engine, inverse_of: :car)
    end

    assert_nothing_raised do
      Class.new(SecondaryActiveRecord::Base).belongs_to(:car, inverse_of: :driver)
    end
  end
...
class InverseHasOneTests < SecondaryActiveRecord::TestCase
  fixtures :men, :faces
  ...
  def test_trying_to_use_inverses_that_dont_exist_should_raise_an_error
    assert_raise(SecondaryActiveRecord::InverseOfAssociationNotFoundError) { Man.first.dirty_face }
  end
end

class InverseHasManyTests < SecondaryActiveRecord::TestCase
  ...
  def test_raise_record_not_found_error_when_invalid_ids_are_passed
    ...
    assert_raise(SecondaryActiveRecord::RecordNotFound) { man.interests.find(invalid_id) }

    invalid_ids = [8432342, 2390102913, 2453245234523452]
    assert_raise(SecondaryActiveRecord::RecordNotFound) { man.interests.find(invalid_ids) }
  end

  def test_raise_record_not_found_error_when_no_ids_are_passed
    man = Man.create!

    exception = assert_raise(SecondaryActiveRecord::RecordNotFound) { man.interests.load.find() }
  ...
  def test_trying_to_use_inverses_that_dont_exist_should_raise_an_error
    assert_raise(SecondaryActiveRecord::InverseOfAssociationNotFoundError) { Man.first.secret_interests }
  end
...
class InverseBelongsToTests < SecondaryActiveRecord::TestCase
  ...
  def test_trying_to_use_inverses_that_dont_exist_should_raise_an_error
    assert_raise(SecondaryActiveRecord::InverseOfAssociationNotFoundError) { Face.first.horrible_man }
  end
end

class InversePolymorphicBelongsToTests < SecondaryActiveRecord::TestCase
  fixtures :men, :faces, :interests
  ...
  def test_trying_to_set_polymorphic_inverses_that_dont_exist_at_all_should_raise_an_error
    ...
    assert_raise(SecondaryActiveRecord::InverseOfAssociationNotFoundError) { Face.first.horrible_polymorphic_man = Man.first }
  end
  ...
  def test_trying_to_set_polymorphic_inverses_that_dont_exist_on_the_instance_being_set_should_raise_an_error
    ...
    assert_raise(SecondaryActiveRecord::InverseOfAssociationNotFoundError) { Face.first.polymorphic_man = Interest.first }
  end
end

# NOTE - these tests might not be meaningful, ripped as they were from the parental_control plugin
# which would guess the inverse rather than look for an explicit configuration option.
class InverseMultipleHasManyInversesForSameModel < SecondaryActiveRecord::TestCase
