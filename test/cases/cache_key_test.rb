...
module SecondaryActiveRecord
  class CacheKeyTest < SecondaryActiveRecord::TestCase
    self.use_transactional_tests = false

    class CacheMe < SecondaryActiveRecord::Base
      self.cache_versioning = false
    end

    class CacheMeWithVersion < SecondaryActiveRecord::Base
    ...
    test "cache_key has no version when versioning is on" do
      ...
      assert_equal "secondary_active_record/cache_key_test/cache_me_with_versions/#{record.id}", record.cache_key
    end
    ...
    test "cache_key_with_version always has both key and version" do
      ...
      assert_equal "secondary_active_record/cache_key_test/cache_me_with_versions/#{r1.id}-#{r1.updated_at.to_s(:usec)}", r1.cache_key_with_version

      r2 = CacheMe.create
      assert_equal "secondary_active_record/cache_key_test/cache_mes/#{r2.id}-#{r2.updated_at.to_s(:usec)}", r2.cache_key_with_version
    end
  end
end
