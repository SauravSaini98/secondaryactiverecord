...
module SecondaryActiveRecord
  class CollectionCacheKeyTest < SecondaryActiveRecord::TestCase
    ...
    test "cache_key for relation" do
      ...
      assert_equal last_developer_timestamp.to_s(SecondaryActiveRecord::Base.cache_timestamp_format), $3
    end
    ...
    test "cache_key for relation with table alias" do
      ...
      table_metadata = SecondaryActiveRecord::TableMetadata.new(Developer, table_alias)
      predicate_builder = SecondaryActiveRecord::PredicateBuilder.new(table_metadata)

      developers = SecondaryActiveRecord::Relation.create(
        Developer,
        table: table_alias,
    ...
    test "cache_key with unknown timestamp column" do
      topics = Topic.where("title like ?", "%Topic%")
      assert_raises(SecondaryActiveRecord::StatementInvalid) { topics.cache_key(:published_at) }
    end

    test "collection proxy provides a cache_key" do
      developers = projects(:secondary_active_record).developers
      assert_match(/\Adevelopers\/query-(\h+)-(\d+)-(\d+)\z/, developers.cache_key)
    end
