...
module SecondaryActiveRecord
  class Migration
    class CompatibilityTest < SecondaryActiveRecord::TestCase
      ...
      def test_migration_doesnt_remove_named_index
        ...
        migration = Class.new(SecondaryActiveRecord::Migration[4.2]) {
          def version; 101 end
          def migrate(x)
        ...
        assert_raise(StandardError) { SecondaryActiveRecord::Migrator.new(:up, [migration]).migrate }
        assert connection.index_exists?(:testings, :foo, name: "custom_index_name")
      end
      ...
      def test_legacy_migrations_raises_exception_when_inherited
        e = assert_raises(StandardError) do
          class_eval("class LegacyMigration < SecondaryActiveRecord::Migration; end")
        end
        assert_match(/LegacyMigration < SecondaryActiveRecord::Migration\[4\.2\]/, e.message)
      end

      if current_adapter?(:PostgreSQLAdapter)
        class Testing < SecondaryActiveRecord::Base
        end

        def test_legacy_change_column_with_null_executes_update
          migration = Class.new(SecondaryActiveRecord::Migration[5.1]) {
            def migrate(x)
...
module LegacyPrimaryKeyTestCases
  ...
  class LegacyPrimaryKey < SecondaryActiveRecord::Base
  end
...
module LegacyPrimaryKeyTest
  class V5_0 < SecondaryActiveRecord::TestCase
    ...
    private
      def migration_class
        SecondaryActiveRecord::Migration[5.0]
      end
  end

  class V4_2 < SecondaryActiveRecord::TestCase
    include LegacyPrimaryKeyTestCases
    ...
    private
      def migration_class
        SecondaryActiveRecord::Migration[4.2]
      end
  end
end
