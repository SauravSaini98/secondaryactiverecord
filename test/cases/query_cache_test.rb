...
class QueryCacheTest < SecondaryActiveRecord::TestCase
  self.use_transactional_tests = false

  fixtures :tasks, :topics, :categories, :posts, :categories_posts

  class ShouldNotHaveExceptionsLogger < SecondaryActiveRecord::LogSubscriber
    attr_reader :logger
  ...
  def teardown
    ...
    SecondaryActiveRecord::Base.connection.disable_query_cache!
    super
  end
  ...
  def test_exceptional_middleware_clears_and_disables_cache_on_error
    ...
    mw = middleware { |env|
      Task.find 1
      Task.find 1
      query_cache = SecondaryActiveRecord::Base.connection.query_cache
  ...
  private def with_temporary_connection_pool
    ...
    new_pool = SecondaryActiveRecord::ConnectionAdapters::ConnectionPool.new SecondaryActiveRecord::Base.connection_pool.spec
    SecondaryActiveRecord::Base.connection_handler.send(:owner_to_pool)["primary"] = new_pool

    yield
  ensure
    SecondaryActiveRecord::Base.connection_handler.send(:owner_to_pool)["primary"] = old_pool
  end
    ...
    with_temporary_connection_pool do
      begin
        if in_memory_db?
          ...
          SecondaryActiveRecord::Base.connection_pool.with_connection do |connection|
            connection.create_table :tasks do |t|
              t.datetime :starting
              t.datetime :ending
            end
          end
          SecondaryActiveRecord::FixtureSet.create_fixtures(self.class.fixture_path, ["tasks"], {}, SecondaryActiveRecord::Base)
        end

        SecondaryActiveRecord::Base.connection_pool.connections.each do |conn|
          assert_cache :off, conn
        end

        assert_not_predicate SecondaryActiveRecord::Base.connection, :nil?
        assert_cache :off
        ...
        middleware {
          ...
          thread_1_connection = SecondaryActiveRecord::Base.connection
          ...
          thread = Thread.new {
            thread_2_connection = SecondaryActiveRecord::Base.connection
        ...
        SecondaryActiveRecord::Base.connection_pool.disconnect!
      end
    end
  end
  ...
  def test_cache_passing_a_relation
    ...
    Post.cache do
      ...
      assert Post.connection.select_all(query).is_a?(SecondaryActiveRecord::Result)
    end
  end
  ...
  def test_cache_does_not_raise_exceptions
    ...
    subscriber = ActiveSupport::Notifications.subscribe "sql.secondary_active_record", logger
  ...
  def test_query_cache_does_not_allow_sql_key_mutation
    subscriber = ActiveSupport::Notifications.subscribe("sql.secondary_active_record") do |_, _, _, _, payload|
  ...
  def test_cache_is_available_when_connection_is_connected
    conf = SecondaryActiveRecord::Base.configurations

    SecondaryActiveRecord::Base.configurations = {}
    Task.cache do
      assert_queries(1) { Task.find(1); Task.find(1) }
    end
  ensure
    SecondaryActiveRecord::Base.configurations = conf
  end
  ...
  def test_cache_is_available_when_using_a_not_connected_connection
    ...
    with_temporary_connection_pool do
      ...
      conf = SecondaryActiveRecord::Base.configurations["arunit"].merge("name" => "test2")
      SecondaryActiveRecord::Base.connection_handler.establish_connection(conf)
      ...
      Task.cache do
        begin
          ...
          SecondaryActiveRecord::Base.connection_handler.remove_connection(Task.connection_specification_name)
  ...
  def test_query_cache_executes_new_queries_within_block
    SecondaryActiveRecord::Base.connection.enable_query_cache!
    ...
    SecondaryActiveRecord::Base.connection.uncached do
  ...
  def test_query_cache_doesnt_leak_cached_results_of_rolled_back_queries
    ...
    Post.transaction do
      ...
      raise SecondaryActiveRecord::Rollback
    end
  ...
  def test_query_cache_does_not_establish_connection_if_unconnected
    with_temporary_connection_pool do
      SecondaryActiveRecord::Base.clear_active_connections!
      assert_not SecondaryActiveRecord::Base.connection_handler.active_connections? # sanity check

      middleware {
        assert_not SecondaryActiveRecord::Base.connection_handler.active_connections?, "QueryCache forced SecondaryActiveRecord::Base to establish a connection in setup"
      }.call({})

      assert_not SecondaryActiveRecord::Base.connection_handler.active_connections?, "QueryCache forced SecondaryActiveRecord::Base to establish a connection in cleanup"
    end
  end
  ...
  def test_query_cache_is_enabled_on_connections_established_after_middleware_runs
    with_temporary_connection_pool do
      ...
      middleware {
        assert_predicate SecondaryActiveRecord::Base.connection, :query_cache_enabled
      }.call({})
      assert_not_predicate SecondaryActiveRecord::Base.connection, :query_cache_enabled
    end
  end
  ...
  def test_query_caching_is_local_to_the_current_thread
    with_temporary_connection_pool do
      ...
      middleware {
        assert SecondaryActiveRecord::Base.connection_pool.query_cache_enabled
        assert SecondaryActiveRecord::Base.connection.query_cache_enabled

        Thread.new {
          assert_not SecondaryActiveRecord::Base.connection_pool.query_cache_enabled
          assert_not SecondaryActiveRecord::Base.connection.query_cache_enabled
        }.join
      }.call({})
  ...
  private
    def middleware(&app)
      ...
      SecondaryActiveRecord::QueryCache.install_executor_hooks executor
      lambda { |env| executor.wrap { app.call(env) } }
    end

    def assert_cache(state, connection = SecondaryActiveRecord::Base.connection)
      case state
      when :off
...
class QueryCacheExpiryTest < SecondaryActiveRecord::TestCase
  ...
  def test_insert
    assert_called(SecondaryActiveRecord::Base.connection, :clear_query_cache, times: 2) do
      Task.cache do
        Task.create!
      end
    end
  ...
  test "threads use the same connection" do
    @connection_1 = SecondaryActiveRecord::Base.connection.object_id

    thread_a = Thread.new do
      @connection_2 = SecondaryActiveRecord::Base.connection.object_id
    end
