...
class RelationTest < SecondaryActiveRecord::TestCase
  ...
  class TopicWithCallbacks < SecondaryActiveRecord::Base
  ...
  def test_scoped
    topics = Topic.all
    assert_kind_of SecondaryActiveRecord::Relation, topics
  ...
  def test_dynamic_find_by_attributes_bang
    ...
    assert_raises(SecondaryActiveRecord::RecordNotFound) { Author.all.find_by_id_and_name!(20, "invalid") }
  end
  ...
  def test_find_id
    ...
    assert_raises(SecondaryActiveRecord::RecordNotFound) { authors.where(name: "lifo").find("42") }
  end
  ...
  def test_find_ids
    ...
    assert_raises(SecondaryActiveRecord::RecordNotFound) { authors.where(name: "lifo").find(authors(:david).id, "42") }
    assert_raises(SecondaryActiveRecord::RecordNotFound) { authors.find(["42", 43]) }
  end
  ...
  def test_delete_all_with_unpermitted_relation_raises_error
    assert_raises(SecondaryActiveRecord::ActiveRecordError) { Author.distinct.delete_all }
    assert_raises(SecondaryActiveRecord::ActiveRecordError) { Author.group(:name).delete_all }
    assert_raises(SecondaryActiveRecord::ActiveRecordError) { Author.having("SUM(id) < 3").delete_all }
  end
  ...
  def test_create_bang
    ...
    assert_raises(SecondaryActiveRecord::RecordInvalid) { birds.create! }
  ...
  def test_first_or_create_bang_with_invalid_options
    assert_raises(SecondaryActiveRecord::RecordInvalid) { Bird.where(color: "green").first_or_create!(pirate_id: 1) }
  end

  def test_first_or_create_bang_with_no_parameters
    assert_raises(SecondaryActiveRecord::RecordInvalid) { Bird.where(color: "green").first_or_create! }
  end
  ...
  def test_first_or_create_with_invalid_array
    assert_raises(SecondaryActiveRecord::RecordInvalid) { Bird.where(color: "green").first_or_create!([ { name: "parrot" }, { pirate_id: 1 } ]) }
  end
  ...
  def test_find_or_create_by!
    assert_raises(SecondaryActiveRecord::RecordInvalid) { Bird.find_or_create_by!(color: "green") }
  end
  ...
  test "loaded relations cannot be mutated by multi value methods" do
    ...
    assert_raises(SecondaryActiveRecord::ImmutableRelation) do
      relation.where! "foo"
    end
  end
  ...
  test "relations with cached arel can't be mutated [internal API]" do
    ...
    assert_raises(SecondaryActiveRecord::ImmutableRelation) { relation.limit!(5) }
    assert_raises(SecondaryActiveRecord::ImmutableRelation) { relation.where!("1 = 2") }
  end

  test "relations show the records in #inspect" do
    relation = Post.limit(2)
    assert_equal "#<SecondaryActiveRecord::Relation [#{Post.limit(2).map(&:inspect).join(', ')}]>", relation.inspect
  end

  test "relations limit the records in #inspect at 10" do
    relation = Post.limit(11)
    assert_equal "#<SecondaryActiveRecord::Relation [#{Post.limit(10).map(&:inspect).join(', ')}, ...]>", relation.inspect
  end
  ...
  test "already-loaded relations don't perform a new query in #inspect" do
    ...
    expected = "#<SecondaryActiveRecord::Relation [#{Post.limit(2).map(&:inspect).join(', ')}]>"
  ...
  private
    def custom_post_relation(alias_name = "omg_posts")
      ...
      SecondaryActiveRecord::Relation.create(
        Post,
        table: table_alias,
