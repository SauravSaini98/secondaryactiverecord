...
class DefaultScopingTest < SecondaryActiveRecord::TestCase
  ...
  def test_sti_association_with_unscoped_not_affected_by_default_scope
    ...
    assert_raises(SecondaryActiveRecord::RecordNotFound) { Post.joins(:special_comments).find(post.id) }
...
class DefaultScopingWithThreadTest < SecondaryActiveRecord::TestCase
