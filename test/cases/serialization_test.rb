...
class SerializationTest < SecondaryActiveRecord::TestCase
  fixtures :books
  ...
  def test_include_root_in_json_is_false_by_default
    assert_equal false, SecondaryActiveRecord::Base.include_root_in_json, "include_root_in_json should be false by default but was not"
  end
  ...
  def test_include_root_in_json_allows_inheritance
    ...
    SecondaryActiveRecord::Base.include_root_in_json = true

    klazz = Class.new(SecondaryActiveRecord::Base)
    klazz.table_name = "topics"
    assert klazz.include_root_in_json

    klazz.include_root_in_json = false
    assert SecondaryActiveRecord::Base.include_root_in_json
    assert !klazz.include_root_in_json
