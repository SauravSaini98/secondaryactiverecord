...
class UnsafeRawSqlTest < SecondaryActiveRecord::TestCase
  ...
  test "order: disallows invalid column name" do
    with_unsafe_raw_sql_disabled do
      assert_raises(SecondaryActiveRecord::UnknownAttributeReference) do
  ...
  def with_config(new_value, &blk)
    old_value = SecondaryActiveRecord::Base.allow_unsafe_raw_sql
    SecondaryActiveRecord::Base.allow_unsafe_raw_sql = new_value
    blk.call
  ensure
    SecondaryActiveRecord::Base.allow_unsafe_raw_sql = old_value
  end
end
